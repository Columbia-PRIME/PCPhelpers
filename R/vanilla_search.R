...
#'
#' \code{vanilla_search} conducts a cross-validated grid search of the parameters for a given data matrix \code{mat}, PCP function \code{pcp_func}, and settings of parameters to search through \code{grid}. See the \strong{Methods} section below for more details.
#'
...
) 
{
  ...
  if (!is.matrix(mat)) stop('Invalid value passed for argument "mat". Argument "mat" must be a matrix. See documentation with "?vanilla_search".')
  if (!(parallel_approach %in% c("sequential", "multisession", "multicore"))) stop('Invalid value passed for argument "parallel_approach". Must be one of: {"sequential", "multisession", "multicore"}. See documentation with "?vanilla_search".')
  ...
  if (perc_test < 0 || perc_test >= 1) stop('Invalid value passed for argument "perc_test". Argument "perc_test" must be in the range [0, 1). See documentation with "?vanilla_search".')
  if (runs < 1) stop('Invalid value passed for argument "runs". Argument "runs" must be >= 1. See documentation with "?vanilla_search".')
  if (perc_test == 0 && runs > 1) stop('Arguments "perc_test" and "runs" are in conflict with one another. Argument "runs" cannot be greater than 1 if "perc_test" is 0. See documentation with "?vanilla_search".')
  if (!is.logical(verbose)) stop('Invalid value passed for argument "verbose". Must be a logical (TRUE / FALSE). See documentation with "?vanilla_search".')
  ...
  # 3. Creating the test sets:
  test_mats <- purrr::map(1:runs, ~ corrupt_mat_randomly2(mat, seed = .x, perc = perc_test))
  
  # 4. Parallel programming setup:
  start_time <- Sys.time()
  if (cores == 1) {
  ...
  if (conserve_memory) {
    pcp_evals <- furrr::future_map_dfr(1:nrow(params), .f = function(i) {
      ...
      tryCatch(expr = {
        ...
        eval_params2(settings = params[i,], test_mat = mat, pcp_mat = pcp_mat, test_mask = test_mats[[params$run_num[i]]]$tilde_mask)
      }, error = function(e) {
    ...
    pcp_evals <- purrr::map_dfr(1:nrow(params), .f = function(i) {
      eval_params2(settings = params[i,], test_mat = mat, pcp_mat = pcp_mats[[i]], test_mask = test_mats[[params$run_num[i]]]$tilde_mask)
    })
  }
  ...
  # 6. Summarizing the results from the search:
  if (runs > 1) {
  ...
  if (conserve_memory) {
    ...
    if ("L_list" %in% names(pcp_mats[[1]])) {
      Ls <- foreach::foreach(k = 1:length(pcp_mats), .combine = c) %do% {
        pcp_mats[[k]]$L_list
      }
      Ss <- foreach::foreach(k = 1:length(pcp_mats), .combine = c) %do% {
        pcp_mats[[k]]$S_list
      }
    } else {
      Ls <- foreach::foreach(k = 1:length(pcp_mats), .combine = c) %do% {
